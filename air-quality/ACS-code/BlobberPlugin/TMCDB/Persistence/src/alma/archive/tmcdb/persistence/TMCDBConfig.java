...
public class TMCDBConfig {
	...
	private String topic = "tmc";
	private String reference = "ACS";
	...
	private String influx_url = "http://200.13.6.10:8086";
    private String aRetentionPolicy = "aRetentionPolicy";
    private Long rectifyTime = 122192928000060000L;
	...
	private void fetchConfiguration() {
		...
		influx_url = dbConfig.get("archive.tmcdb.monitoring.influx_url");
		if (influx_url == null) {
			influx_url = "http://localhost:8086";
		}
		aRetentionPolicy = dbConfig.get("archive.tmcdb.monitoring.aRetentionPolicy");
		if (aRetentionPolicy == null) {
			aRetentionPolicy = "autogen";
		}
		rectifyTime = Long.parseLong(dbConfig.get("archive.tmcdb.monitoring.rectifyTime"));
		if (rectifyTime == null) {
			rectifyTime = 122192928000060000L;
		}
		influx_user = dbConfig.get("archive.tmcdb.monitoring.influx_user");
		if (influx_user == null) {
			influx_user = "root";
		}
		...
		reference = dbConfig.get("archive.tmcdb.monitoring.reference");
		if (reference == null) {
			reference = "ACS";
		}
		topic = dbConfig.get("archive.tmcdb.monitoring.topic");
		if (topic == null) {
			topic = "tmc";
		}
