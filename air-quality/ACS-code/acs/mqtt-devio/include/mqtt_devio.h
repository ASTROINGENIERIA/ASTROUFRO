...
namespace mqtt
{
    ...
    class acs_callbacklong : public virtual mqtt::callback

    {
        ...
        std::string& topic_;
        mqtt_deviolong* devio_;

        public:
        acs_callbacklong(mqtt::async_client& cli, std::string& topic, mqtt_deviolong* devio);
    };
...
//                            int topicLen, MQTTClient_message* m);
    ...
    class mqtt_deviolong: public virtual DevIO<CORBA::LongLong>
    {
        friend class acs_callbacklong;
        public:
        mqtt_deviolong(const std::string& mqtt_brk_addr,
                const std::string& topic, const std::string& client_name);
        virtual ~mqtt_deviolong();

        virtual bool initializeValue();
        virtual CORBA::LongLong read(ACS::Time& timestamp);

        virtual void write(const CORBA::LongLong& value, ACS::Time& timestamp);
...
//
    ...
    class mqtt_readlong: public mqtt_deviolong
    {
        public:
        mqtt_readlong(const std::string& mqtt_brk_addr, const std::string& topic, const std::string& client_name);
        virtual ~mqtt_readlong();
